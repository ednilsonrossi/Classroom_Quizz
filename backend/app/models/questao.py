...
class TipoQuestao(enum.Enum):
    ...
    def from_string(cls, tipo_str):
        ...
        for membro in cls: #Membro são os valores da própria classe
            if membro.name.lower() == tipo_str.lower() or membro.value.lower() == tipo_str.lower():
                return membro
    
        raise ValueError(f"O tipo '{tipo_str}' não foi encontrado no Enum TipoQuestao.")
...
class Questao(db.Model):
    ...
    correction_img = db.Column(db.String(255), nullable=True)
    ...
    def __init__(self, usuario_id, quiz_id, texto, tipo_pergunta, descricao=None, ponto=10, tempo=60, ordem=1, image_path=None, correction=None, correction_img=None):
        ...
        self.correction_img = correction_img